...
FILE_PATTERNS = {
    2010: r'st.*cou\.txt$',
    2020: r'st.*cou2020\.txt$'
}

# The 2020 files already include headers, so we set it to None.
HEADERS = {
    2010: "STATE|STATEFP|COUNTYFP|COUNTYNAME|CLASSFP",
    2020: None
}
...
for year in YEARS:
    ...
    links = [link.get('href') for link in soup.find_all('a') if link.get('href') and pattern.match(link.get('href'))]
